package com.swcguild.movingimages;
...
/**
 * Created by warde on 3/21/15.
 */
public class AnimatedView extends ImageView {

    // the Handler allows us to send messages to the thread that will run our animation
    private Handler h;
    private final int FRAME_RATE = 30;
    // holds all the images we want to display
    List<MovingImage> images = new ArrayList<>();
    // thread on which our animation will run
    // invalidate is a method on View (ImageView extends View) - it invalidates the entire
    // view in preparation for it being drawn again
    ...
    public AnimatedView(Context context, AttributeSet attrs) {
        ...
        img.xVelocity = 5;
        img.yVelocity = 5;
        img.image = (BitmapDrawable) context.getResources().getDrawable(R.drawable.android);
        images.add(img);
